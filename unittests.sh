...
for UNITTEST in $(find . -name '*_test.go' -a -not -path '*/nfs/test/e2e/*' -prune -a -not -path '*/snapshot/pkg/cloudprovider/providers/aws/*' -prune -a -not -path '*/openstack/standalone-cinder/pkg/provisioner/*' -prune -a -not -path '*/vendor/*' -prune)
do
